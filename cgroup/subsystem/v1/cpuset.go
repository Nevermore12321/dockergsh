...
func (css *CpuSetSubSystem) Set(cgroupPath string, resConf *subsystem.ResourceConfig) error {
	...
	if cpusetSubSystemCgroupPath, err := GetCgroupPath(css.Name(), cgroupPath, true); err != nil {
		return err
	} else {
		// 设置 对应 cgroup 的 cpu 资源限制，也就是修改 cpuset.cpus 文件
		if resConf.CpuShare != "" {
			if err := ioutil.WriteFile(path.Join(cpusetSubSystemCgroupPath, "cpuset.cpus"), []byte(resConf.CpuSet), 0644); err != nil {
...
func (css *CpuSetSubSystem) Apply(cgroupPath string, pid int) error {
	if cpusetSubSystemCgroupPath, err := GetCgroupPath(css.Name(), cgroupPath, false); err != nil {
		return fmt.Errorf("get cgroup %s error: %v", cgroupPath, err)
	} else {
		// 将进程号 pid 写入到 cgroup 的虚拟文件系统对应目录下的 tasks 文件中
		if err := ioutil.WriteFile(path.Join(cpusetSubSystemCgroupPath, "tasks"), []byte(strconv.Itoa(pid)), 0644); err != nil {
